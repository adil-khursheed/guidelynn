...
interface MultiSelectProps
  ...
  /**
   * If true, allows creating custom items when no matching option is found.
   * Optional, defaults to false.
   */
  allowCustomItems?: boolean;

  /**
   * Callback function triggered when a custom item is created.
   * Receives the new item value and returns a MultiSelectOption.
   * Optional, defaults to a function that creates a basic option with the value as label.
   */
  onCustomItemCreate?: (value: string) => MultiSelectOption;

  /**
   * Custom text for the "Create new item" button.
   * Optional, defaults to "Create: {searchValue}".
   */
  createItemText?: string | ((searchValue: string) => string);
}
...
export const MultiSelect = React.forwardRef<MultiSelectRef, MultiSelectProps>(
  (
    ...
    const getAllOptions = React.useCallback((): MultiSelectOption[] => {
      if (options.length === 0 && customOptions.length === 0) return [];
      ...
      if (isGroupedOptions(options)) {
        ...
        allOptions = [...options, ...customOptions];
      }
    ...
    }, [options, customOptions, deduplicateOptions, isGroupedOptions]);
    ...
    const filteredOptions = React.useMemo(() => {
      ...
      // Check if we have any matching options
      let hasMatchingOptions = filteredCustomOptions.length > 0;

      // Filter regular options
      let filteredRegularOptions;
      if (isGroupedOptions(options)) {
        filteredRegularOptions = options
          .map((group) => ({
            ...group,
        ...
        hasMatchingOptions =
          hasMatchingOptions || filteredRegularOptions.length > 0;

        // If we have custom options and grouped options, add custom options as a special group
        if (filteredCustomOptions.length > 0) {
          return [
            ...filteredRegularOptions,
            { heading: "Custom Options", options: filteredCustomOptions },
          ];
        }
      ...
      const filteredFlatOptions = options.filter(
        (option) =>
          option.label.toLowerCase().includes(searchValue.toLowerCase()) ||
          option.value.toLowerCase().includes(searchValue.toLowerCase())
      );

      hasMatchingOptions = hasMatchingOptions || filteredFlatOptions.length > 0;

      // Return combined filtered options
      return [...filteredFlatOptions, ...filteredCustomOptions];
    }, [options, customOptions, searchValue, searchable, isGroupedOptions]);
    ...
    const handleInputKeyDown = (
      ...
      if (event.key === "Enter") {
        if (
          allowCustomItems &&
          searchValue &&
          !getAllOptions().some((opt) => opt.value === searchValue)
        ) {
    ...
    const handleCreateCustomItem = () => {
      if (!searchValue || disabled) return;

      // Check if this value already exists
      const existingOption = getAllOptions().find(
        (option) => option.value === searchValue || option.label === searchValue
      );

      if (existingOption) {
        // If it exists, just select it
        if (!selectedValues.includes(existingOption.value)) {
          const newSelectedValues = [...selectedValues, existingOption.value];
          setSelectedValues(newSelectedValues);
          onValueChange(newSelectedValues);
        }
        return;
      ...
      const newOption = onCustomItemCreate
        ? onCustomItemCreate(searchValue)
        : {
            label: searchValue,
            value: searchValue,
            icon: WandSparkles,
      ...
      announce(`Created and selected new item: ${newOption.label}`, "polite");
    };
        ...
        <Popover
          ...
          <PopoverTrigger asChild>
            <Button
              ...
              className={cn(
                "flex px-3 py-1 text-base shadow-xs rounded-md border dark:bg-input/30 border-input min-h-9 h-auto items-center justify-between bg-inherit hover:bg-inherit [&_svg]:pointer-events-auto w-full min-w-0",
                  ...
                  {allowCustomItems && searchValue ? (
                    <div className="flex flex-col gap-2 p-1">
                      ...
                      <Button
                        ...
                        <span>
                          {typeof createItemText === "function"
                            ? createItemText(searchValue)
                            : createItemText || `Create: ${searchValue}`}
                        </span>
                      </Button>
                    </div>
                  ) : (
